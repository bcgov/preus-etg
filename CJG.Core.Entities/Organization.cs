...
namespace CJG.Core.Entities
{
	...
	public class Organization : EntityBase
	{
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			if (this.OrganizationType == null && this.OrganizationTypeId == 0)
			...
			// Must be associated with a LegalStructure.
			if (this.LegalStructure == null && this.LegalStructureId == 0)
				yield return new ValidationResult("The organization must be associated to a legal structure.", new[] { nameof(this.LegalStructure) });

			// Must have a YearEstablished no older than 250 years and no more than one year in the future.
			if (this.YearEstablished < AppDateTime.UtcNow.AddYears(-250).Year || this.YearEstablished > AppDateTime.UtcNow.AddYears(1).Year)
			...
			if (this.NumberOfEmployeesWorldwide < this.NumberOfEmployeesInBC)
			...
			if (this.AnnualEmployeesTrained < 0 || this.AnnualEmployeesTrained > this.NumberOfEmployeesWorldwide)
