...
namespace CJG.Core.Entities
{
    ...
    public class GrantApplicationStateChange : EntityBase
    {
        ...
        public GrantApplicationStateChange(GrantApplication grantApplication, ApplicationStateInternal fromState, ApplicationStateInternal toState, InternalUser user, string reason)
        {
            ...
            FromState = fromState;
            ToState = toState;
            Assessor = user ?? throw new ArgumentNullException(nameof(user));
            AssessorId = user.Id;
            Reason = reason;
            ChangedDate = AppDateTime.UtcNow;
        }

        public GrantApplicationStateChange(GrantApplication grantApplication, ApplicationStateInternal fromState, ApplicationStateInternal toState, string reason)
        {
            GrantApplication = grantApplication ?? throw new ArgumentNullException(nameof(grantApplication));
            GrantApplicationId = grantApplication.Id;
            FromState = fromState;
        ...
        public GrantApplicationStateChange(GrantApplication grantApplication, ApplicationStateInternal fromState, ApplicationStateInternal toState, User user, string reason)
        {
            ...
            FromState = fromState;
            ToState = toState;
            ApplicationAdministrator = user ?? throw new ArgumentNullException(nameof(user));
            ApplicationAdministratorId = user.Id;
            Reason = reason;
            ChangedDate = AppDateTime.UtcNow;
        }
