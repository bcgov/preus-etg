...
namespace CJG.Core.Entities
{
	...
	public class TrainingCost : EntityBase
	{
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			// TotalEstimatedReimbursement must not be greater than sum of EligibleCosts.
			var totalEstimatedReimbursement = this.EligibleCosts.Sum(ec => ec.EstimatedReimbursement);
			if (totalEstimatedReimbursement != this.TotalEstimatedReimbursement)
			...
			if (totalAgreedCommitment != this.AgreedCommitment)
			...
			if (this.EligibleCosts.Any())
			{
				...
				if (this.EstimatedParticipants > 0 && totalEstimatedReimbursement > maxReimbursementAmount * this.EstimatedParticipants)
