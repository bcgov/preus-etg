...
namespace CJG.Core.Entities
{
	...
	public class GrantOpening : EntityBase
	{
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			if (entry.State == EntityState.Added)
			{
				...
				if (!((this.PublishDate >= AppDateTime.UtcMorning &&
					  ...
					  (this.PublishDate <= AppDateTime.UtcMorning &&
					   ...
					   this.ClosingDate < AppDateTime.UtcNow)))
				{
					yield return new ValidationResult("Publish date, opening date and closing date must either all in the past or all in the future.", new[] { nameof(this.State) });
				}
				...
				switch (this.State)
				{
					...
					case GrantOpeningStates.Published:
						...
						if (!(this.PublishDate <= AppDateTime.UtcNow && this.OpeningDate > AppDateTime.UtcNow))
							yield return new ValidationResult("Published date must be today or in the past, opening date must be in the future while in open state.", new[] { nameof(this.ClosingDate) });
						break;
					// to OPEN
			...
			if (this.PublishDate < earliest_publish_date
			    || this.PublishDate > this.TrainingPeriod?.EndDate)
			...
			if (this.OpeningDate < this.PublishDate
			    || this.OpeningDate.ToLocalTime().Date > this.ClosingDate.ToLocalTime().Date)
			...
			if (this.ClosingDate < this.OpeningDate
			    || this.ClosingDate.Date > this.TrainingPeriod?.EndDate
			    || this.ClosingDate.Date < this.TrainingPeriod?.StartDate)
