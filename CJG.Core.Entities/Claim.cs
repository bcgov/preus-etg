...
namespace CJG.Core.Entities
{
	...
	public class Claim : EntityBase
	{
		...
		/// <summary>
		/// Was the applicant note updated? If yes, create a Log Note and reset this property.
		/// </summary>
		public bool ApplicantUpdatedNotes { get; set; }
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			// Refresh EF cache.
			context.Set<ClaimEligibleCost>()
				.Where(ec => ec.ClaimId == Id && ec.ClaimVersion == ClaimVersion)
				.Include(ec => ec.ParticipantCosts)
				.Include(ec => ec.EligibleCost)
				.ToList();
			context.Set<TrainingProgram>()
				.Where(tp => tp.GrantApplicationId == GrantApplicationId)
				.Include(tp => tp.GrantApplication)
				.Include(tp => tp.GrantApplication.TrainingCost)
				.ToList();
			context.Set<GrantApplication>()
				.Where(tp => tp.Id == GrantApplicationId)
				.Include(ga => ga.GrantOpening.GrantStream.GrantProgram)
				.Include(tp => tp.TrainingCost)
				.ToList();
