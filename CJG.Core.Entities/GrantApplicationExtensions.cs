...
namespace CJG.Core.Entities
{
	...
	public static class GrantApplicationExtensions
	{
		...
		{
			var services = grantApplication.TrainingCost.EligibleCosts.Where(ec => ec.EligibleExpenseType.ServiceCategory?.ServiceTypeId.In(ServiceTypes.SkillsTraining, ServiceTypes.EmploymentServicesAndSupports) ?? false);
		...
		{
			return grantApplication.ApplicationStateInternal.IsValidWorkflowTrigger(ApplicationWorkflowTrigger.ValidateTrainingProvider)
				&& (grantApplication.TrainingPrograms.Any(p => p.TrainingProviders.Any(tp => tp.TrainingProviderInventoryId == null))
					|| grantApplication.TrainingProviders.Any(tp => tp.TrainingProviderInventoryId == null));
		}
		...
		/// <summary>
		/// Check if the grant application is submitable.
		/// </summary>
		/// <param name="grantApplication"></param>
		/// <returns></returns>
		public static bool IsSubmitable(this GrantApplication grantApplication)
		{
			// If attachments are required they must be provided.
			if (grantApplication.GrantOpening.GrantStream.AttachmentsIsEnabled && grantApplication.GrantOpening.GrantStream.AttachmentsRequired && grantApplication.Attachments.Count() == 0)
				return false;
			...
			switch (grantApplication.GrantOpening.GrantStream.GrantProgram.ProgramTypeId)
			{
				case (ProgramTypes.WDAService):
					...
					var minProviders = grantApplication.GrantOpening.GrantStream.ProgramConfiguration.EligibleExpenseTypes.Where(eet => eet.IsActive && eet.ServiceCategory.ServiceTypeId == ServiceTypes.EmploymentServicesAndSupports && eet.MinProviders > 0).Sum(eet => eet.MinProviders);
		...
		/// <summary>
		/// Get the total agreed committment of all training programs in this grant application.
		/// </summary>
		/// <param name="grantApplication"></param>
		/// <returns></returns>
		...
		{
			var enabled = grantApplication.GrantOpening?.GrantStream?.EligibilityEnabled ?? false;
			var required = grantApplication.GrantOpening?.GrantStream?.EligibilityRequired ?? true;
			return !enabled || !required || grantApplication.EligibilityConfirmed;
		}
		...
		{
			if (grantApplication?.InvitationExpiresOn < AppDateTime.UtcNow) return true;
			if (grantApplication.CanReportParticipants) return false;
		...
		{
			var dateSubmitted = (grantApplication.DateSubmitted ??
				(grantApplication.ApplicationStateInternal > ApplicationStateInternal.Draft ?
				grantApplication.DateAdded : AppDateTime.UtcNow)).ToLocalMorning().Date;
