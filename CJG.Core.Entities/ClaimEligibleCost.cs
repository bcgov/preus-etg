...
namespace CJG.Core.Entities
{
	...
	public class ClaimEligibleCost : EntityBase
	{
		...
		public ClaimEligibleCost(Claim claim, EligibleCost eligibleCost) : this(claim)
		{
			...
			{
				foreach (var participant in claim.GrantApplication.ParticipantForms.Where(pf => !pf.IsExcludedFromClaim))
				{
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			if (this.ClaimParticipants > maxClaimParticipants)
			...
			if (this.AssessedReimbursementCost > agreedMaximumReibursement)
				yield return new ValidationResult($"The assessed government contribution cannot exeed the agreed maximum of {agreedMaximumReibursement:C}", new[] { nameof(this.AssessedReimbursementCost) });
			...
			if (maxGovContribution > remainingGovContribution)
			...
			if (this.EligibleExpenseTypeId == (int)ExpenseTypes.ParticipantAssigned)
			{
				...
				if (this.ClaimMaxParticipantCost != validClaimParticipantCost)
				...
				if (this.ClaimMaxParticipantReimbursementCost != validClaimParticipantReimbursement)
				...
				if (this.AssessedMaxParticipantReimbursementCost > maxAssessedParticipantReimbursement)
			...
			if (this.ClaimMaxParticipantCost != this.ClaimMaxParticipantReimbursementCost + this.ClaimParticipantEmployerContribution)
			...
			if (this.ParticipantCosts.Any(pc => pc.ClaimParticipantCost > validClaimParticipantCost || pc.ClaimReimbursement > validClaimParticipantReimbursement))
			{
			...
			{
				if (this.AssessedCost > remainingToAssessed)
			...
			if (this.AssessedMaxParticipantCost != validAssessedParticipantCost)
			...
			if ((this.AssessedMaxParticipantCost > 0 && this.AssessedMaxParticipantReimbursementCost > this.AssessedMaxParticipantCost) || (this.AssessedMaxParticipantCost < 0 && this.AssessedMaxParticipantReimbursementCost < this.AssessedMaxParticipantCost))
			...
			if (this.AssessedMaxParticipantCost != this.AssessedMaxParticipantReimbursementCost + this.AssessedParticipantEmployerContribution)
			...
			// Each participant cost must be less than or equal to the maximum amounts.
			if (this.ParticipantCosts.Any(pc => pc.AssessedParticipantCost > maxAssessedParticipantCost || pc.AssessedReimbursement > maxAssessedParticipantReimbursement || pc.AssessedReimbursement > this.AssessedMaxParticipantReimbursementCost))
			{
				var eligibleExpenseType = context.Set<EligibleExpenseType>().FirstOrDefault(eet => eet.Id == this.EligibleExpenseTypeId);
