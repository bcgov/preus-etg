...
namespace CJG.Core.Entities
{
	...
	public class ParticipantForm : EntityBase
	{
		...
		/// <summary>
		/// get/set - The foreign key to the martial status of this participant.
		/// </summary>
		public int? MartialStatusId { get; set; }

		/// <summary>
		/// get/set - The martial status of this participant.
		/// </summary>
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			if (this.BirthDate.Date < oldest || this.BirthDate.Date > youngest)
				yield return new ValidationResult($"The birthdate must be between '{oldest:yyyy}' and '{youngest:yyyy}'.", new[] { nameof(this.BirthDate) });

			if (entry.State == EntityState.Added)
			{
				// ConsentDateEntered must be today.
				if (this.ConsentDateEntered.Date != AppDateTime.UtcNow.Date)
					yield return new ValidationResult($"The consent date must be today '{AppDateTime.UtcNow:yyyy-MM-dd}'.", new[] { nameof(this.ConsentDateEntered) });
			}
			else if (entry.State == EntityState.Modified)
			{
				// ConsentDateEntered must be equal to the original ConsentDateEntered (no changes)
				var original_consentDateEntered = (DateTime)entry.OriginalValues[nameof(this.ConsentDateEntered)];
				if (this.ConsentDateEntered.Date != original_consentDateEntered.Date)
					yield return new ValidationResult($"The consent date cannot be changed from '{original_consentDateEntered:yyyy-MM-dd}'.", new[] { nameof(this.ConsentDateEntered) });
			}
