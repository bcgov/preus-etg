...
namespace CJG.Core.Entities
{
	...
	public class GrantApplication : EntityBase
	{
		...
		/// <summary>
		/// get/set - Foreign key to the applicantion completion report.
		/// </summary>
		...
		public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			else if (entry.State == EntityState.Modified)
			{
				...
				else if (this.ApplicationStateExternal == ApplicationStateExternal.Submitted)
				{
					if (originalExternalState != ApplicationStateExternal.Submitted)
					{
						...
						else if (this.ApplicationStateInternal != ApplicationStateInternal.ReturnedToAssessment && !this.IsSubmitable())
				...
				{
					var reason = this.GetStateChange(ApplicationStateInternal.ClaimReturnedToApplicant)?.Reason ?? this.GetCurrentClaim()?.ClaimAssessmentNotes;
				...
				{
					// Must validate training providers if internal state change
					if (originalApplicationStateInternal != this.ApplicationStateInternal && trainingPrograms.Any(tp => tp.TrainingProvider?.TrainingProviderInventoryId == null))
			...
			// If eligibility confirmation is required by the stream then the applicant must confirm it in the application.
			if (this.IsWDAService() && !this.EligibilityConfirmed())
				yield return new ValidationResult($"Applicants must meet these eligibility requirements to apply for the grant under this stream.", new[] { nameof(this.EligibilityConfirmed) });
