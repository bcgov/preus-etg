...
namespace CJG.Testing.UnitTests.ApplicationServices
{
	...
	{		
		...
		public void CreateUserProfile_Should_Add_User_And_Commit()
		{
			...
			// Act
			service.CreateUserProfile(user.BCeIDGuid, userProfileDetails, preferencePrograms);
		...
		public void UpdateUserProfile_Should_Update_User_Twice_And_Commit_Zero_Times()
		{
			...
			// Act
			service.UpdateUserProfile(user.Id, userProfileDetails, preferencePrograms);
		...
		[TestMethod, TestCategory("User"), TestCategory("Service")]
		public void UpdateGrantProgramPreferences_When_User_UserPreference_And_UserGrantProgramPreferences_Are_Null_Create_And_Assign_To_User()
		{
			// Arrange
			var user = EntityHelper.CreateExternalUser();
			var preferencePrograms = new List<UserGrantProgramPreferenceModel>();
			var helper = new ServiceHelper(typeof(UserService), user);
			var mockUsers = helper.MockDbSet(user);
			var service = helper.Create<UserService>();

			// Act
			service.UpdateGrantProgramPreferences(user.Id, preferencePrograms, true);

			// Assert
			user.UserPreference.Should().NotBeNull();
			user.UserGrantProgramPreferences.Should().NotBeNull();
			helper.GetMock<IDataContext>().Verify(x => x.CommitTransaction(), Times.Exactly(1));
		}

		[TestMethod, TestCategory("User"), TestCategory("Service")]
		public void UpdateGrantProgramPreferences_When_Not_StandAloneTransaction_Should_Not_Commit()
		{
			// Arrange
			var user = EntityHelper.CreateExternalUser();
			var preferencePrograms = new List<UserGrantProgramPreferenceModel>();
			...
			// Act
			service.UpdateGrantProgramPreferences(user.Id, preferencePrograms, false);

			// Assert
			helper.GetMock<IDataContext>().Verify(x => x.CommitTransaction(), Times.Exactly(0));
		}
		...
		[TestMethod, TestCategory("User"), TestCategory("Service")]
		public void GetPreferencePrograms_When_GrantProgram_Count_Is_One_Returns_One_PreferencePrograms()
		{
			// Arrange
			var externalUser = EntityHelper.CreateExternalUser();
			var identity = HttpHelper.CreateIdentity(externalUser);
			...
			var service = helper.Create<UserService>();
			var programs = new GrantProgram() {
				Id = 1,
				Name = "Program",
				EligibilityDescription = "Program Description",
				State = GrantProgramStates.Implemented
			};
			var programPref = new List<UserGrantProgramPreference> { new UserGrantProgramPreference() { GrantProgram = programs }  };
			var user = new User()
			{
				Id = 1,
				UserGrantProgramPreferences = programPref
			};
			helper.MockDbSet(user);
			var dbContextMock = helper.GetMock<IDataContext>();
			var dbSetMock = helper.MockDbSet(programs);
			dbContextMock.Setup(x => x.GrantPrograms.AsNoTracking()).Returns(dbSetMock.Object);

			// Act
			var result = service.GetPreferencePrograms(user.Id);

			// Assert
			result.Count.Should().Be(1);
			result.FirstOrDefault().ProgramId.Equals(programs.Id);
			result.FirstOrDefault().Name.Equals(programs.Name);
			result.FirstOrDefault().Description.Equals(programs.EligibilityDescription);
		}

		[TestMethod, TestCategory("User"), TestCategory("Service")]
		public void GetPreferencePrograms_One_Existing_UserGrantProgram_Under_User_Returns_One_UserPreferenceProgram()
		{
			// Arrange
			var externalUser = EntityHelper.CreateExternalUser();
			var identity = HttpHelper.CreateIdentity(externalUser);
			...
			{
				Id = 1,
				UserGrantProgramPreferences = new List<UserGrantProgramPreference>
				{
					new UserGrantProgramPreference
					{
						UserId = 1,
						GrantProgram = new GrantProgram {Id = 1 }
					}
				}
			};
		...
		[TestMethod, TestCategory("User"), TestCategory("Service")]
		public void GetPreferencePrograms_When_GrantPrograms_And_User_UserGrantProgramPreferences_Empty_Return_Zero_GrantProgramPreferences()
		{
			// Arrange
			var externalUser = EntityHelper.CreateExternalUser();
			var identity = HttpHelper.CreateIdentity(externalUser);
		...
		[TestMethod, TestCategory("User"), TestCategory("Service")]
		public void MustSelectGrantProgramPreferences_When_User_UserPreference_Is_Null_Return_True()
		{
			// Arrange
			var externalUser = EntityHelper.CreateExternalUser();
			var identity = HttpHelper.CreateIdentity(externalUser);
			...
			// Act
			var result = service.MustSelectGrantProgramPreferences(user.Id);
