...
using CJG.Infrastructure.NotificationService.ServiceMocks;
using NLog;
...
namespace CJG.Infrastructure.NotificationService
{
	...
	internal class AppFactory
	{
		...
		private IContainer CreateContainer()
		{
			...
			builder.RegisterType<ReturnUnassessedApplicationsJob>().As<IReturnUnassessedApplicationsJob>();
			...
			// These need to be registered for the WorkflowJobStateMachine in the ReturnUnassessedApplicationsJob
			builder.RegisterType<GrantApplicationJobService>().As<IGrantApplicationJobService>();
			builder.RegisterType<GrantOpeningService>().As<IGrantOpeningService>();
			builder.RegisterType<NoteService>().As<INoteService>();
			builder.RegisterType<UserService>().As<IUserService>();
			builder.RegisterType<SiteMinderService>().As<ISiteMinderService>();
			builder.RegisterType<StaticDataService>().As<IStaticDataService>();

			builder.RegisterType<MockedIBCeIDService>().As<IBCeIDService>();

			builder.RegisterType<ApplicationUserManager>();
			builder.RegisterType<UserManagerAdapter>().As<IUserManagerAdapter>();
		...
		public IReturnUnassessedApplicationsJob GetReturnUnassessedApplicationsJob()
		{
			return Container.Resolve<IReturnUnassessedApplicationsJob>();
		}
