    ...
    return $scope.confirmDialog('Delete Grant Stream', 'Do you want to delete the "' + $scope.model.Name + '" grant stream?')
      .then(function () {
        return $scope.ajax({
          ...
          .then(function () {
            // Delete program
            var index = $scope.grantStreams.findIndex(function (item) { return item == $scope.section.selectedStream; });
            $scope.grantStreams.splice(index, 1);
            $scope.section.selectedStream = null;
            $scope.model = null;
          });
      })
      .catch(angular.noop);
