app.controller('PrioritizationManagementHighOpportunityOccupations', function ($scope, $attrs, $controller) {
  $scope.section = {
    name: 'PrioritizationManagementHighOpportunityOccupations'
  };
  ...
  $scope.selectFile = function () {
    var $input = angular.element('#hoo-import-file');
    $input.click();
  }
  ...
  $scope.updateIndustries = function ($event) {
    if ($scope.upload.file) {
      return $scope.ajax({
        url: '/Int/Admin/Prioritization/UpdateHOO',
          method: 'POST',
          dataType: 'file',
          data: {
  ...
  function loadScores() {
    return $scope.load({
      url: '/Int/Admin/Prioritization/HOO',
      set: 'scores'
    });
  }

  $scope.getScores = function (pageKeyword, page, quantity) {
    var useUrl = '/Int/Admin/Prioritization/HOO' + '?sortby=' + $scope.sort.column + '&sortDesc=' + $scope.sort.descending;
    return $scope.ajax({
        url: useUrl
      })
