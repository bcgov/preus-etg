app.controller('WorkQueueFilter', function ($scope, $attrs, $controller, $timeout, Utils, ngDialog) {
  ...
  function init() {
    ...
    initFilter(filter, 'TrainingPeriodId', function (attr) { return attr ? parseInt(attr.Value) || null : filter['TrainingPeriodId']; });
  ...
  function initAttributes(filter) {
    ...
    findAttribute(filter, 'TrainingPeriodId', true).Value = filter.TrainingPeriodId;
  ...
  function findAttribute(filter, key, create) {
    ...
    if (attribute || !create) return attribute;
  ...
  $scope.toggleStateGroup = function ($event) {
    ...
    if (element.hasClass('open')) element.removeClass('open');
  ...
  $scope.loadTrainingPeriods = function () {
    return loadGrantStreams($scope.model.FiscalYearId)
      .catch(angular.noop);
  }
