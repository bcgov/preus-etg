...
app.controller('WorkQueue', function ($scope, $attrs, $controller, $timeout, Utils, ngDialog) {
  $scope.section = {
    name: 'WorkQueue',
    onRefresh: function () {
  ...
  function loadFiscalYears() {
    return $scope.load({
      ...
      .then(function (response) {
        return loadTrainingPeriods($scope.filter.FiscalYearId || response.data[0].Key);
      });
  }
  ...
  function loadTrainingPeriods(fiscalYearId) {
    if (fiscalYearId) {
      return $scope.load({
        url: '/Int/Work/Queue/Training/Periods/' + fiscalYearId,
        set: 'trainingPeriods',
  ...
  function filterChanged() {
    if ($scope.filter.AssessorId !== $scope.current.AssessorId
      ...
      || $scope.filter.TrainingPeriodId !== $scope.current.TrainingPeriodId
