app.controller('ApplicationNote', function ($scope, $controller, $timeout) {
    ...
    $scope.changeAttachment = function () {
      return $scope.attachmentDialog('Change Attachment', $scope.ngDialogData.note.Attachment, false, '.pdf,.doc,.docx,.xlsx,.xls,.eml,.msg,.jpg,.png,.gif,.jpeg')
    ...
    $scope.addAttachment = function () {
      return $scope.attachmentDialog('Add Attachment', { Id: 0 }, false, '.pdf,.doc,.docx,.xlsx,.xls,.eml,.msg,.jpg,.png,.gif,.jpeg')
          .then(function (attachment) {
              mapAttachmentToNote(attachment);
          })
          .catch(angular.noop);
    ...
    $scope.tinymceOptions = {
      plugins: 'link code autoresize preview fullscreen lists advlist anchor powerpaste',
