app.controller('RegionalExceptions', function ($scope, $attrs, $controller, $timeout, Utils, ngDialog) {
  $scope.section = {
    name: 'RegionalExceptions',
    onRefresh: function () {
  ...
  /**
   * Make AJAX request for region data
   * @function loadRegions
   * @returns {Promise}
   **/
  function loadRegions() {
    return $scope.load({
      url: '/Int/RegionalExceptions/Regions',
      set: 'regions',
      condition: !$scope.regions || !$scope.regions.length
    });
  }
  ...
  function loadFiscalYears() {
    return $scope.load({
      url: '/Int/RegionalExceptions/Fiscal/Years',
      set: 'fiscalYears',
  ...
  function loadTrainingPeriods(fiscalYearId, grantStreamId) {
    if (fiscalYearId) {
      ...
      let url = '/Int/RegionalExceptions/Training/Periods/' + fiscalYearId;
      if (grantStreamId)
  ...
  function loadGrantPrograms() {
    return $scope.load({
      url: '/Int/RegionalExceptions/Grant/Programs',
      set: 'grantPrograms',
  ...
  function loadGrantStreams(grantProgramId) {
    return $scope.load({
      url: '/Int/RegionalExceptions/Grant/Streams/' + (grantProgramId || ''),
      set: 'grantStreams',
  ...
  function loadApplications(page, quantity) {
    ...
    return $scope.load({
      url: '/Int/RegionalExceptions/Queue?page=' + page + '&quantity=' + quantity,
      method: 'POST',
      data: $scope.filter,
      set: 'model',
  ...
  $scope.openRegionDialog = function (application) {
    //$scope.selectedGrantApplicationId = application.Id;
    return ngDialog.openConfirm({
        template: '/Int/RegionalExceptions/SelectRegion/View',
        data: {
          title: 'Selection Region for Exception',
      ...
      .then(function (selectedRegionId) {
      return $scope.ajax({
        url: '/Int/RegionalExceptions/SelectRegion',
            method: 'PUT',
            data: function () {
              return {
                grantApplicationId: application.Id,
          ...
          .then(function() {
            window.location = '/Int/RegionalExceptions/View';
          });
      })
      .catch(angular.noop);
  }
