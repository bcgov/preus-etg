 ...
 **/
  function loadTrainingHistory() {    
    return $scope.load({
      url: '/Int/Application/Participant/Info/View/92/TrainingHistory',
      set: 'model'
    }).catch(angular.noop);
  }
 ...
 **/
  $scope.getTrainingHistory = function (pageKeyword, page, quantity) {
    var useUrl = '/Int/Application/Participant/Info/View/92/TrainingHistory/' + page + '/' + quantity + '?sortby=' + $scope.sort.column + '&sortDesc=' + $scope.sort.descending + (pageKeyword ? '&search=' + pageKeyword : '');

    //var useUrl = '/Int/Application/Participant/Info/View/92/TrainingHistory';
    return $scope.ajax({
      url: useUrl
    })
