app.controller('ApplicationBatchApproval', function ($scope, $attrs, $controller, $timeout, Utils, ngDialog) {
  ...
  function loadFiscalYears() {
    return $scope.load({
      ...
      .then(function (response) {
        ...
        return loadTrainingPeriods($scope.filter.FiscalYearId);
      });
  }
   ...
   * @returns {Promise}
   **/
  function loadTrainingPeriods(fiscalYearId) {
    if (fiscalYearId) {
      return $scope.load({
        url: '/Int/Application/Batch/Approval/Training/Periods/' + fiscalYearId,
        set: 'trainingPeriods',
        condition: !$scope.trainingPeriods || !$scope.trainingPeriods.length || fiscalYearId !== $scope.current.FiscalYearId
      })
        .then(function () {
  ...
  $scope.loadTrainingPeriods = function () {
    return loadTrainingPeriods($scope.filter.FiscalYearId).catch(angular.noop);
  }
  ...
  $scope.validFilter = function () {
    return $scope.filter.TrainingPeriodId && $scope.filter.GrantProgramId && $scope.filter.GrantStreamId;
  }
  ...
  $scope.clearFilter = function() {
    ...
    $scope.filter.TrainingPeriodId = null;
  }
      ...
      return loadApplications(page, quantity)
        .then(function () {
          $scope.current = {
            ...
            TrainingPeriodId: $scope.filter.TrainingPeriodId,
  ...
  /**
   * Toggle all of the filtered applictions to either selected or not selected.
   * @function toggleAll
