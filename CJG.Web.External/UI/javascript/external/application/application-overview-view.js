 ...
 function loadApplication() {
  ...
  $scope.Populate = function (type, populateTypes) {
    ...
    if (type === types.SELECTION) {
      ...
      ) && ($scope.model.TrainingProvider.CourseOutlineDocument === null || $scope.model.TrainingProvider.ProofOfQualificationsDocument == null)) {
        return populateTypes.INCOMPLETE;
  ...
  $scope.PopulateSection = function (process, populateTypes) {
    if (process.ServiceCategoryTypeId === utils.ServiceTypes.SkillsTraining) {
      ...
      var matching = $.grep(process.TrainingPrograms, function (n, i) {
        return n.TrainingProgramState < 1 ||
          (n.AssociatedProvider.TrainingProviderType.PrivateSectorValidationType
            && (n.AssociatedProvider.CourseOutlineDocument === null
              || n.AssociatedProvider.ProofOfQualificationsDocument == null));
      });
  ...
  $scope.PopulateSubSection = function (target, type, populateTypes) {
    if ((type === types.PROGRAM && target.TrainingProgramState == utils.TrainingProgramStates.Incomplete) ||
      (type === types.PROVIDER && target.TrainingProviderState == utils.TrainingProviderStates.Incomplete)) {
      return populateTypes.INCOMPLETE;
    } else if (
      (type === types.PROGRAM && target.AssociatedProvider.TrainingProviderType.PrivateSectorValidationType
        && (target.AssociatedProvider.ProofOfQualificationsDocument === null || target.AssociatedProvider.CourseOutlineDocument === null))
      || (type === types.PROVIDER && target.TrainingProviderType.PrivateSectorValidationType
        && (target.ProofOfQualificationsDocument === null || target.CourseOutlineDocument === null))) {
      return populateTypes.INCOMPLETE;
    } else if (type === types.PROGRAM ||
      (type === types.PROVIDER && target.TrainingProviderState == utils.TrainingProviderStates.Complete)) {
      return populateTypes.COMPLETE;
    }
  }
