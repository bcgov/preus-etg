	...
	public class InformationController : BaseController
	{
		...
		private void PrepareStep1(ref ParticipantInfoViewModel model, Guid invitationKey)
		{
			...
			model.ParticipantInfoStep1ViewModel = new ParticipantInfoStep1ViewModel
			{
				...
				ProgramDescription = grantApplication.GetProgramType() == ProgramTypes.EmployerGrant ? grantApplication.TrainingPrograms.FirstOrDefault().CourseTitle : grantApplication.ProgramDescription.Description,
		...
		private void PrimeStep3(ref ParticipantInfoViewModel model)
		{
			...
			model.ParticipantInfoStep3ViewModel.MartialStatuses = _staticDataService.GetMartialStatuses().Select(x => new KeyValuePair<int, string>(x.Id, x.Caption)).ToArray();
		...
		private void SaveParticipantInfo(ParticipantInfoViewModel model)
		{
			try
			{
				...
				var newParticipantForm = new ParticipantForm
				{
					...
					MartialStatusId = model.ParticipantInfoStep3ViewModel.MartialStatusId != 0 ? model.ParticipantInfoStep3ViewModel.MartialStatusId : null,
					...
					CanadaImmigrant = model.ParticipantInfoStep3ViewModel.CanadaImmigrant.Value,
					YearToCanada = model.ParticipantInfoStep3ViewModel.YearToCanada,
					CanadaRefugee = model.ParticipantInfoStep3ViewModel.CanadaRefugee.Value,
