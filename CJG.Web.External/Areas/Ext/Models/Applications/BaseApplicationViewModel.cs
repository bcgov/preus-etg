...
namespace CJG.Web.External.Areas.Ext.Models
{
	public class BaseApplicationViewModel : BaseViewModel
	{
		...
		public bool EnableBusinessCase { get; set; } = false;
		public bool BusinessCaseRequired { get; set; } = false;
		public string BusinessCaseHeader { get; set; }
		public int BusinessCaseState { get; set; }
		public bool HasBusinessCase { get; }
		public List<ParticipantReporting.ParticipantViewModel> Participants { get; set; }
		...
		public BaseApplicationViewModel(GrantApplication grantApplication)
		{
			...
			this.EnableBusinessCase = grantApplication.GrantOpening.GrantStream.BusinessCaseIsEnabled;
			this.BusinessCaseRequired = grantApplication.GrantOpening.GrantStream.BusinessCaseRequired;
			this.BusinessCaseHeader = grantApplication.GrantOpening.GrantStream.BusinessCaseExternalHeader;
			this.BusinessCaseState = this.BusinessCaseRequired && grantApplication.ApplicationStateExternal == ApplicationStateExternal.ApplicationWithdrawn ? 1 : (grantApplication.BusinessCaseDocument != null) ? 2 : 0;
			this.HasBusinessCase = grantApplication.BusinessCaseDocument != null;

			ShowEligibility = grantApplication.CanViewParticipantEligibilty();
			this.Participants = new List<ParticipantReporting.ParticipantViewModel>();
			foreach (var participantForm in grantApplication.ParticipantForms)
            {
				this.Participants.Add(new ParticipantReporting.ParticipantViewModel(participantForm, ShowEligibility));
			}

			this.PIFCompletionConfirmed = grantApplication.PIFCompletionConfirmed();			
			...
			if (this.ProgramType == ProgramTypes.EmployerGrant)
			{
				...
				{
					this.TrainingProgram = new OverviewTrainingProgram(trainingProgram);
				}
