...
namespace CJG.Web.External.Areas.Ext.Models.TrainingPrograms
{
    public class TrainingProgramViewModel : BaseViewModel
	{
		...
		[Required(ErrorMessage = "If you have expected qualifications you must include the title of the qualification."), MaxLength(255)]
		...
		public TrainingProgramViewModel(TrainingProgram trainingProgram)
		{
			...
			if (trainingProgram.GrantApplication == null)
				throw new ArgumentNullException($"{nameof(trainingProgram)}.{nameof(trainingProgram.GrantApplication)}");
													 ...
													 HttpPostedFileBase[] files = null)
		{
			...
			if (SelectedDeliveryMethodIds != null && SelectedDeliveryMethodIds.Any())
			{
				...
				if (SelectedDeliveryMethodIds.Contains(Constants.Delivery_Classroom) || SelectedDeliveryMethodIds.Contains(Constants.Delivery_Workplace))
                {
					...
					// If the Training Provider was added first, it won't get be coupled to the TrainingProgram and won't get an incomplete status
					if (trainingProgram.TrainingProvider == null && grantApplication.TrainingProviders.Any())
					{
						var trainingProvider = grantApplication.TrainingProviders.FirstOrDefault();
						if (trainingProvider != null && trainingProvider.TrainingAddress == null)
							grantApplication.TrainingProviders
								.Where(tp => tp.Id == trainingProvider.Id)
								.ForEach(tp => tp.TrainingProviderState = TrainingProviderStates.Incomplete);
					}
                }
			...
			trainingProgram.TitleOfQualification = new[] { Constants.ExpectedQualifications_None }.Contains(ExpectedQualificationId.GetValueOrDefault())
				? null
			...
			if (new[] { Constants.SkillsFocus_ApprenticeshipTraining }.Contains(SkillFocusId.GetValueOrDefault()))
			{
				...
				{
					var thisIds = SelectedUnderRepresentedGroupIds.ToList();
					var currentIds = trainingProgram.UnderRepresentedGroups.Select(dm => dm.Id).ToList();
					var removeIds = currentIds.Except(thisIds).ToList();
					var addIds = thisIds.Except(currentIds).Except(removeIds);
