	...
	public class UserProfileController : BaseController
	{
		...
		public JsonResult GetUserProfile(int id)
		{
			...
			try
			{
				...
				viewModel.PreferencePrograms = _userService.GetPreferencePrograms(id).Where(x => x.IsImplemented).ToList();

				// select default preference program
				if (id == 0 && viewModel.PreferencePrograms.Count > 0 && !viewModel.PreferencePrograms.Any(x => x.IsSelected))
					viewModel.PreferencePrograms.First().IsSelected = true;
			}
			catch (Exception ex)
			{
		...
		public JsonResult CreateUserProfile(UserProfileViewModel viewModel)
		{
			try
			{
				...
				// PreferencePrograms is required
				if (!viewModel.PreferencePrograms.Any(x => x.IsSelected))
				{
					ModelState.AddModelError("PreferencePrograms", "You must select one or more grant programs");
				}

				if (ModelState.IsValid)
				{
					_userService.CreateUserProfile(_siteMinderService.CurrentUserGuid, viewModel.UserProfileDetails, viewModel.PreferencePrograms);
				}
				else
				{
		...
		public JsonResult UpdateUserProfile(UserProfileViewModel viewModel)
		{
			try
			{
				...
				if (ModelState.IsValid)
				{
					_userService.UpdateUserProfile(viewModel.UserId, viewModel.UserProfileDetails, viewModel.PreferencePrograms);
				}
				else
				{
		...
		#region Grant Program Preference Endpoints
		/// <summary>
		/// View the user's grant program perferences page.
		/// </summary>
		/// <returns></returns>
		[Route("User/Profile/Program/Preferences/View")]
		public ActionResult GrantProgramPreferencesView()
		{
			return View();
		}

		/// <summary>
		/// Get the data for the user's grant program preference page.
		/// </summary>
		/// <returns></returns>
		[HttpGet]
		[Route("User/Profile/Program/Preferences")]
		public JsonResult GetGrantProgramPreferences()
		{
			var viewModel = new UserProfileViewModel();
			try
			{
				...
				viewModel.PreferencePrograms = _userService.GetPreferencePrograms(currentUser.Id).Where(x => x.IsImplemented).ToList();
			}
			catch (Exception ex)
			{
				HandleAngularException(ex, viewModel);
		...
		/// <summary>
		/// Update the user's grant program preferences in the datasource.
		/// </summary>
		/// <param name="viewModel"></param>
		/// <returns></returns>
		[HttpPut]
		...
		[PreventSpam]
		[Route("User/Profile/Program/Preferences")]
		public JsonResult UpdateGrantProgramPreferences(UserProfileViewModel viewModel)
		{
			try
			{
				_userService.UpdateGrantProgramPreferences(viewModel.UserId, viewModel.PreferencePrograms);
				viewModel.ReturnURL = "/Ext/Home/Index";
			}
			catch (Exception ex)
			{
				HandleAngularException(ex, viewModel);
