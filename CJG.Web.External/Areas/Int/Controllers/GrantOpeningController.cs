...
namespace CJG.Web.External.Areas.Int.Controllers
{
	...
	public class GrantOpeningController : BaseController
	{
		...
		public JsonResult GetGrantOpenings(int fiscalId, int grantProgramId)
		{
			...
			try
			{
				if (fiscalId > 0 && grantProgramId > 0)
					viewModel = new FiscalGrantOpeningViewModel(fiscalId, grantProgramId, _grantOpeningService, _grantProgramService, _grantStreamService, _staticDataService);
			}
			catch (Exception ex)
			{
		...
		/// THIS IS A HORRIBLE IMPLEMENTION
		/// </summary>
		...
		public JsonResult GetInitialGrantOpening(int trainingPeriodId, int grantStreamId)
		{
			...
			try
			{
				...
				else
				{
					...
					var trainingPeriod = _staticDataService.GetTrainingPeriod(trainingPeriodId) ?? throw new NoContentException();
		...
		private GrantOpening AddOrUpdate(GrantOpeningViewModel model)
		{
			...
			var trainingPeriod = _staticDataService.GetTrainingPeriod(model.TrainingPeriodId);
			...
			if (grantStream.DateFirstUsed == null) { grantStream.DateFirstUsed = trainingPeriod.StartDate; }
