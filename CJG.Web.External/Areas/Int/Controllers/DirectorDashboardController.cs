...
namespace CJG.Web.External.Areas.Int.Controllers
{
	...
	public class DirectorDashboardController : BaseController
	{
		...
		[HttpGet]
		[Route("Application/Batch/Approval/Training/Periods/{fiscalYearId:int}")]
		public JsonResult GetTrainingPeriods(int fiscalYearId)
		{
			IEnumerable<KeyValuePair<int, string>> results = new KeyValuePair<int, string>[0];
			try
			{
				var trainingPeriods = _fiscalYearService.GetTrainingPeriods(fiscalYearId);
				results = trainingPeriods.Select(tp => new KeyValuePair<int, string>(tp.Id, tp.Caption)).ToArray();
			}
			catch (Exception ex)
			{
				HandleAngularException(ex);
