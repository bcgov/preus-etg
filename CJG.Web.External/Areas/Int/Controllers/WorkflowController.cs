	...
	public class WorkflowController : BaseController
	{
		...
		public JsonResult ReturnToDraft(WorkflowViewModel model)
		{
			try
			{
				var grantApplication = _grantApplicationService.Get(model.ApplicationWorkflowViewModel.Id);
				grantApplication.RowVersion = Convert.FromBase64String(model.ApplicationWorkflowViewModel.RowVersion);
				_grantApplicationService.ReturnToDraft(grantApplication);
				model = new WorkflowViewModel(grantApplication, User, x => Url.RouteUrl(x));
			}
			catch (Exception ex)
			{
		...
		[PreventSpam]
		[ValidateRequestHeader]
		[HttpPut, Route("Workflow/Return/To/New", Name = "ReturnUnderAssessmentToNew")]
		public JsonResult ReturnUnderAssessmentToNew(WorkflowViewModel model)
		{
			try
			{
				var grantApplication = _grantApplicationService.Get(model.ApplicationWorkflowViewModel.Id);
				grantApplication.RowVersion = Convert.FromBase64String(model.ApplicationWorkflowViewModel.RowVersion);
				_grantApplicationService.ReturnUnderAssessmentToNew(grantApplication);
				model = new WorkflowViewModel(grantApplication, User, x => Url.RouteUrl(x));
			}
			catch (Exception ex)
			{
