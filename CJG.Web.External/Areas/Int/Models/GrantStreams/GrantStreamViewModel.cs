...
namespace CJG.Web.External.Areas.Int.Models.GrantStreams
{
	public class GrantStreamViewModel : BaseViewModel
	{
		...
		public GrantStreamViewModel(GrantStream grantStream, IServiceCategoryService serviceCategoryService, IStaticDataService staticDataService, IGrantStreamService grantStreamService)
		{
			...
			this.RowVersion = Convert.ToBase64String(grantStream.RowVersion);
			this.ProgramType = grantStream.GrantProgram.ProgramTypeId;
			this.GrantProgramState = grantStream.GrantProgram.State;
			this.ProgramConfigurationId = grantStream.ProgramConfigurationId;
			this.AccountCodeId = grantStream.AccountCodeId;

			this.EligibilityRequirements = HttpUtility.HtmlDecode(HttpUtility.UrlDecode(this.EligibilityRequirements));
			this.SelfAccountCode = grantStream.AccountCodeId != grantStream.GrantProgram.AccountCodeId;
			this.SelfProgramConfiguration = grantStream.ProgramConfigurationId != grantStream.GrantProgram.ProgramConfigurationId;
			this.HasApplications = grantStreamService.HasApplications(grantStream.Id);

			this.CanDelete = grantStream.DateFirstUsed == null && !grantStream.IsActive && grantStream.GrantOpenings.Count == 0;
		}
		#endregion
	}
}
