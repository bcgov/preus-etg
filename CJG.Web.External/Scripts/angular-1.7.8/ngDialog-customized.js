            ...
            function ($document, $templateCache, $compile, $q, $http, $rootScope, $timeout, $window, $controller, $injector) {
                ...
                var privateMethods = {
                    ...
                    performCloseDialog: function ($dialog, value) {
                        ...
                        $elements.body.css('position', '');
                        $elements.body.css('top', '');
                        window.scrollTo(0, scope.scrollY || 0);
                ...
                var publicMethods = {
                    ...
                    open: function (opts) {
                        ...
                        $q.all({
                            ...
                            var content = '<div class="ngdialog-content" role="document" style="cursor: move; resize: both;" ng-style="ngDialogStyle"'
                                + ' ng-mousedown="ngDialogMouseDown($event)" ng-mousemove="ngDialogMouseMove($event)" ng-mouseup="ngDialogMouseUp($event)">'
                                + '<div class="ngdialog-content-wrapper" style="cursor: default;" ng-mousedown="$event.stopPropagation()">'
                                + template + '</div></div>';

                            $dialog = $el('<div id="' + dialogID + '" class="ngdialog' + hasOverlayClass + '"></div>');
                            $dialog.html(options.overlay ? '<div class="ngdialog-overlay" ng-mousemove="ngDialogMouseMove($event)"></div>' + content : content);
                            $dialog.data('$ngDialogOptions', options);

                            scope.ngDialogPosition = {
                                x: 0,
                                y: 0
                            };
                            ...
                            scope.ngDialogMouseDown = function ($event) {
                                var cornerX = $event.currentTarget.clientWidth - $event.offsetX;
                                var cornerY = $event.currentTarget.clientHeight - $event.offsetY;
                                if (cornerX > 15 || cornerY > 15) {
                                    scope.ngDialogDrag = {
                                        x: scope.ngDialogPosition.x,
                                        y: scope.ngDialogPosition.y,
                                        originX: $event.pageX,
                                        originY: $event.pageY
                                    };
                                }
                            ...
                            scope.ngDialogMouseMove = function ($event) {
                                if (scope.ngDialogDrag) {
                                    scope.ngDialogPosition.x = scope.ngDialogDrag.x + $event.pageX - scope.ngDialogDrag.originX;
                                    scope.ngDialogPosition.y = scope.ngDialogDrag.y + $event.pageY - scope.ngDialogDrag.originY;
                                    scope.ngDialogStyle["margin-left"] = scope.ngDialogPosition.x + "px";
                                    scope.ngDialogStyle["margin-top"] = scope.ngDialogPosition.y + "px";
                                }
                            };

                            scope.ngDialogMouseUp = function ($event) {
                                scope.ngDialogDrag = null;
                            };
                            ...
                            $timeout(function () {
                                ...
                                $elements.body.css('position', 'fixed');
                                $elements.body.css('top', '-' + scope.scrollY + 'px');
                            });
                    ...
                    openConfirm: function (opts) {
                        if (angular.element(".ngdialog-content").length !== 0) {
                            angular.element(".ngdialog-content")[0].style.zIndex = 10001;
                            angular.element(".ngdialog-overlay")[0].style.zIndex = 10000;
                        }
                        var defer = $q.defer();
                        var options = angular.copy(defaults);
