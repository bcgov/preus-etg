    ...
    public class IntakeManagementBuilder
	{
		...
		private Dictionary<int, IntakeManagementViewModel.GrantOpeningIntakeViewModel> LoadGrantOpeningIntakes(GrantOpening grantOpening, List<GrantApplication> grantApplications, TrainingBudgetModel trainingPeriodBudget)
		{
			var intakeModels = new Dictionary<int, IntakeManagementViewModel.GrantOpeningIntakeViewModel>
			{
				...
				{
					11, GetIntakeModelForClaimsNotReceived(grantApplications, "Claim Not Yet Received")
				},
				{
		...
		private static IntakeManagementViewModel.GrantOpeningIntakeViewModel GetIntakeModelForClaimsNotReceived(IEnumerable<GrantApplication> grantApplications, string intakeGroupName)
		{
			var claimsNotYetReceivedAgreed = GetIntakeModelFor(grantApplications, intakeGroupName, CommitmentType.Agreed,
				new List<ApplicationStateInternal>
				{
					ApplicationStateInternal.AgreementAccepted,
					ApplicationStateInternal.OfferIssued
				});

			var claimsNotYetReceivedRequested = GetIntakeModelFor(grantApplications, intakeGroupName, CommitmentType.Requested,
				new List<ApplicationStateInternal>
				{
					ApplicationStateInternal.ClaimReturnedToApplicant
				});

			claimsNotYetReceivedAgreed.Number += claimsNotYetReceivedRequested.Number;
			claimsNotYetReceivedAgreed.Value += claimsNotYetReceivedRequested.Value;

			return claimsNotYetReceivedAgreed;
		}
