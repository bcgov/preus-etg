...
PRINT 'UPDATE DEFAULT AGREEMENT COVERLETTER TEMPLATE'
...
<br />
<p>Attention:</p>
<p>Based upon your application for funding under the @Model.GrantOpening.GrantStream.GrantProgram.Name program
	<strong>("ETG")</strong>, we are pleased to inform you that we have approved the Training Provider, Training and Eligible Training
...
<p>Thank you for your participation in the @Model.GrantOpening.GrantStream.GrantProgram.Name program. The Province of British Columbia introduced
	the @Model.GrantOpening.GrantStream.GrantProgram.Name to help better align skills training with employer needs, create more jobs for Canadians
...
',
Title = N'Default Applicant Cover Letter Template'
where id = 1;

SET IDENTITY_INSERT DocumentTemplates ON

-- Agreement Schedule A
PRINT 'INSERT DEFAULT AGREEMENT SCHEDULE A TEMPLATE'
INSERT INTO [dbo].[DocumentTemplates] ([Id], [DocumentType], [Title], [Body], [IsActive], [DateAdded], [DateUpdated]) VALUES (2, 2, N'Default Applicant Schedule A Template',
N'
@model CJG.Core.Entities.GrantApplication
...
PRINT 'INSERT DEFAULT AGREEMENT SCHEDULE B TEMPLATE'
INSERT INTO [dbo].[DocumentTemplates] ([Id], [DocumentType], [Title], [Body], [IsActive], [DateAdded], [DateUpdated]) VALUES (3, 3, N'Default Applicant Schedule B Template',
N'
 @model CJG.Core.Entities.GrantApplication
...
<ul class="list--nostyle">
	...
	<li>&ldquo;<b>Program Requirements</b>&rdquo; means the general principles, criteria, requirements and Participant and Employer expectations
		...
		time to time, which can be viewed via
		<a href="https://www.workbc.ca/Employer-Resources/BC-Employer-Training-Grant.aspx"
		    target="_blank">About @Model.GrantOpening.GrantStream.GrantProgram.Name</a> or such other location as we may specify from time to time;</li>
	...
	<li>&ldquo;<b>Term</b>&rdquo; means the period that begins upon the return of a signed copy of the Approval Letter by the Employer to
		the Province and ends on the end date set out in Schedule A or March 31, 2020, whichever is earlier; and</li>
...
<ol class="list--lettered">
	...
	<li>comply with any &ldquo;Marketing, Publicity and Communications Guidelines&rdquo; regarding any public announcements that
		...
		<a href="mailto:@(@Model.GrantOpening.GrantStream.GrantProgram.ProgramCode)info@gov.bc.ca">@(@Model.GrantOpening.GrantStream.GrantProgram.ProgramCode)info@gov.bc.ca</a> or such other address as we may specify from time to time; </li>
	...
	<li> establish and maintain complete and accurate accounting and administrative records with respect to the Participants, the
		...
		to us upon request for a period of at least seven years following the completion of the Training.
		<br>
		<br> We reserve the right to contact Participants, trainers or any other person in order to substantiate reimbursement requests,
		Training activities, records or other matters pertaining to your obligations or your participation in the Program. </li>
</ol>
...
<ol class="list--lettered">
	<li> to you, to your mailing address or email address as shown in the Approval Letter
...
<p>
	...
	copy attached in PDF format) to an email address provided by the other Party for such purpose.</p>
...
PRINT 'INSERT DEFAULT APPLICANT DECLARATION TEMPLATE'
INSERT INTO [dbo].[DocumentTemplates] ([Id], [DocumentType], [Title], [Body], [IsActive], [DateAdded], [DateUpdated]) VALUES (4, 4, N'Default Applicant Declaration Template',
N'
@model CJG.Core.Entities.GrantApplication 
...
<li>I acknowledge that I have read and understand the @Model.GrantOpening.GrantStream.GrantProgram.Name criteria applicable to this Application, including the sample @Model.GrantOpening.GrantStream.GrantProgram.Name Agreement, consisting of the Approval Letter, Schedule A and Schedule B (and the Program Requirements referred to therein), as made available by the Province of British Columbia at the link below;</li> 
<li>I acknowledge that, as the terms and conditions of the @Model.GrantOpening.GrantStream.GrantProgram.Name Agreement are subject to change from time to time, should this Application be approved, the @Model.GrantOpening.GrantStream.GrantProgram.Name Agreement that will be sent to the Applicant for signature may materially differ from the sample B.C. Employer Training Grant Agreement that was posted at the time this Application was submitted and I acknowledge that I (or another individual authorized by the Applicant) will be responsible for reviewing, understanding and agreeing to the terms and conditions as they appear at the time the Applicant enters into a B.C. Employer Training Grant Agreement with the Province;</li> 
...
-- Participant Consent
PRINT 'INSERT DEFAULT PARTICIPANT CONSENT TEMPLATE'
INSERT INTO [dbo].[DocumentTemplates] ([Id], [DocumentType], [Title], [Body], [IsActive], [DateAdded], [DateUpdated]) VALUES (5, 5, N'Default Participant Consent Template',
N'
 @model CJG.Core.Entities.GrantApplication
<h2 class="subheader">Personal Information</h2>
<p> The Government of Canada ("Canada") provides funding for the @Model.GrantOpening.GrantStream.GrantProgram.Name ("@Model.GrantOpening.GrantStream.GrantProgram.ProgramCode") under the Canada-British
	Columbia Workforce Development Agreement (WDA), administered by the British Columbia Ministry of Advanced Education, Skills
	and Training ("AEST"), and under the Labour Market Development Agreement ("LMDA"), administered by the British Columbia
	Ministry of Social Development and Poverty Reduction ("SDPR"). </p>
<p> All personal information related to your participation in a ETG training opportunity ("Personal Information") is collected
	pursuant to sections 26(c) and 26(e) of the Freedom of Information and Protection of Privacy Act ("FOIPPA"). This information
	will be used for administrative, evaluation, research, accountability, and reporting purposes, including to determine your
	eligibility for participation in the ETG program and to meet federal reporting requirements under the WDA and LMDA. </p>
<p> Under the authority of Section 33.1(d) and 33.2(d) of FOIPPA, the personal information collected may be disclosed to the
	Department of Employment and Skills Development Canada ("ESDC") or the Canada Employment Insurance Commission ("CEIC") for
	the purposes of administering the Employment Insurance Act. The CEIC may also use your Personal Information for policy analysis,
	research or evaluation purposes. </p>
<p> Under the authority of Section 3.6 of the LMDA, if you are currently in receipt of Employment Insurance (EI) benefits, a
	referral under Section 25 of the EI Act may be placed on your EI claim, to allow you to continue to receive EI benefits,
	up to the end of your EI benefit period, while you participate in the @Model.GrantOpening.GrantStream.GrantProgram.Name program. </p>
<p> All ETG training participants are required to complete two satisfaction surveys at approximately 3 months and 12 months following
	completion of their training ("Surveys"). The Surveys will ask basic questions about the outcomes of training and whether
	the training met your employment needs. Your contact information will be shared with British Columbia"s statistical agency,
	BC Stats, in order for them to contact you to conduct these Surveys. </p>
<p> You may also be asked if you wish to, or you may volunteer to, provide a testimonial regarding your ETG training experience
	("Testimonial"). Testimonials, and any Personal Information that you choose to include in a Testimonial, may be used and
	disclosed to the public to promote the ETG program. </p>
<h2>Consent and Certification</h2>
<p> Effective as of the date set out below, and in consideration of the opportunity for me to participate in ETG training, I:
	</p>
<ul>
	...
	<li>certify that I understand that I am expected to complete my training and I must complete the Surveys in order for my employer
		to meet all of its @Model.GrantOpening.GrantStream.GrantProgram.Name Agreement obligations;</li>
</ul>
<p> By confirming my consent, I acknowledge and agree that this typed signature has the same legal effect as a written signature.
	</p>
<hr>
<p> If you have any questions about the collection, use or disclosure of your Personal Information, please contact the Program
	Manager, @Model.GrantOpening.GrantStream.GrantProgram.Name at 1-877-952-6914, or by mail at: Program Manager, @Model.GrantOpening.GrantStream.GrantProgram.Name, 2nd
	Floor, 1106 Cook Street, Victoria, BC, V8V 3Z9 or by submitting an email to
	<a href="mailto:ETGInfo@gov.bc.ca">ETGInfo@gov.bc.ca</a>
</p>
',1, SYSDATETIME(), SYSDATETIME());
