PRINT 'Insert Default CWRG Applicant Schedule A Template INTO [DocumentTemplates] and  [GrantApplicationDeliveryPartnerServices]'


INSERT INTO DocumentTemplates ([DocumentType],[Title],[Body],[IsActive],[DateAdded],[DateUpdated],[Description])
VALUES(
	2
	, 'Default CWRG Applicant Schedule A Template'
...
<div class="form--readonly">
			...
			@foreach (var EligibleCost in Model.TrainingCost.EligibleCosts.Where(ec => ((ec.AgreedMaxCost > 0) && ec.EligibleExpenseType.ServiceCategory.ServiceTypeId == ServiceTypes.EmploymentServicesAndSupports)))
			{
				<tr>
					...
					<td>
						...
						{
							@(Breakdown.EligibleExpenseBreakdown.Caption)
							<br />
						}
					</td>
				</tr>
			...
			@foreach (var expenseItem in OrderedList)
			{
				if (showAgreedCosts)
				{
					<tr data-expense-id="@expenseItem.EligibleExpenseType.Id"
						...
						<td>
							@(expenseItem.EligibleExpenseType.Caption)
						</td>
  ...
  SET ApplicantScheduleATemplateId = SCOPE_IDENTITY()  WHERE id =3