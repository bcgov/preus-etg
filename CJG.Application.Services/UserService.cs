...
namespace CJG.Application.Services
{
	...
	public class UserService : Service, IUserService
	{
		...
		#region Methods
		/// <summary>
		/// Determine whether the user needs to select their Grant Program preferences
		/// </summary>
		/// <param name="userId"></param>
		/// <returns></returns>
		public bool MustSelectGrantProgramPreferences(int userId)
		{
			var user = _dbContext.Users.Find(userId);
			if (user.UserPreference?.GrantProgramPreferencesUpdated == null) {
				return true;
			}

			var grantPrograms = _dbContext.GrantPrograms.ToList();
			if (grantPrograms.Any(x => x.DateImplemented != null &&
									x.DateImplemented > user.UserPreference.GrantProgramPreferencesUpdated &&
									(user.UserGrantProgramPreferences.Any(y => y.IsSelected && y.GrantProgramId == x.Id) || !user.UserGrantProgramPreferences.Any(y => y.GrantProgramId == x.Id)))) { 
				return true;
			}
		...
		/// <param name="userId"></param>
		/// <returns></returns>
		public List<UserGrantProgramPreferenceModel> GetPreferencePrograms(int userId)
		{
			List<UserGrantProgramPreferenceModel> programs = new List<UserGrantProgramPreferenceModel>();
			var user = _dbContext.Users.Find(userId);

			var existIds = user != null ? user.UserGrantProgramPreferences.Select(x => x.GrantProgram.Id).Distinct().ToList() : new List<int>();

			var updatePrograms = _dbContext.GrantPrograms
										 .AsNoTracking().Where(x => x.State == GrantProgramStates.Implemented || existIds.Contains(x.Id)).ToList();

			programs.AddRange(updatePrograms.Select(x => new UserGrantProgramPreferenceModel()
			{
				ProgramId = x.Id,
				Name = x.Name,
				Description = x.EligibilityDescription,
				IsSelected = user != null ? user.UserGrantProgramPreferences.Where(z => z.GrantProgramId == x.Id).Select(z => z.IsSelected).FirstOrDefault() : false,
				IsNew = !existIds.Contains(x.Id),
				IsImplemented = x.State == GrantProgramStates.Implemented
			}));

			return programs.OrderBy(x => x.Name).ToList();
		}
		...
		/// <param name="preferencePrograms"></param>
		public void CreateUserProfile(Guid guid, UserProfileDetailModel userProfileDetails, List<UserGrantProgramPreferenceModel> preferencePrograms)
		{
			var newUser = GetBCeIDUser(guid);
			if (newUser == null) throw new NoContentException(nameof(newUser));
			...
			{
				GrantProgramPreferencesUpdated = AppDateTime.UtcNow
			};
			newUser.UserPreference = userPreference;

			foreach (var p in preferencePrograms)
			{
				var preference = new UserGrantProgramPreference
				{
					IsSelected = p.IsSelected,
					GrantProgramId = p.ProgramId
				};
				newUser.UserGrantProgramPreferences.Add(preference);
			}
		...
		/// <param name="userProfileDetails"></param>
		/// <param name="preferencePrograms"></param>
		public void UpdateUserProfile(int userId, UserProfileDetailModel userProfileDetails, List<UserGrantProgramPreferenceModel> preferencePrograms)
		{
			UpdateUserDetails(userId, userProfileDetails, false);
			UpdateGrantProgramPreferences(userId, preferencePrograms, false);
			_dbContext.CommitTransaction();
		}
		...
		/// <summary>
		/// Update User Grant Program Preferences
		/// </summary>
		/// <param name="preferencePrograms"></param>
		/// <param name="userId"></param>
		public void UpdateGrantProgramPreferences(int userId, List<UserGrantProgramPreferenceModel> preferencePrograms, bool standAloneTransaction = true)
		{
			var user = _dbContext.Users.Find(userId);
			if (user == null) throw new NoContentException(nameof(user));
			...
			{
				user.UserPreference.GrantProgramPreferencesUpdated = AppDateTime.UtcNow;
			}

			foreach (var p in preferencePrograms)
			{
				var preference = user.UserGrantProgramPreferences.Where(x => x.GrantProgramId == p.ProgramId).FirstOrDefault();
				if (preference != null)
				{
					preference.IsSelected = p.IsSelected;
				}
				else
				{
					preference = new UserGrantProgramPreference
					{
						IsSelected = p.IsSelected,
						GrantProgramId = p.ProgramId
					};
					user.UserGrantProgramPreferences.Add(preference);
				}
			}
			_dbContext.Update(user);
			if (standAloneTransaction)
		...
		{
			//_dbContext.ClaimParticipants.Any();
		...
		/// </summary>
		public void UpdateUserFromBCeIDAccount(User currentUser = null)
		{
			if (currentUser == null)
			...
			if (!simulator)
			{
				...
				if (bceIdUser == null) throw new InvalidOperationException($"Can\'t find BCeID user based on user GUID: {_siteMinderService.CurrentUserGuid}");
		...
		public bool SyncUserFromBCeIDAccount(User user)
		{
			...
			if (bceIdUser == null) throw new InvalidOperationException($"Can't find BCeID user based on user GUID: {user.BCeIDGuid}");
			if (user == null) throw new ArgumentNullException($"Can't find user based on user GUID: {user.BCeIDGuid}");
		...
		{
			if (bceidUser == null) throw new ArgumentNullException(nameof(bceidUser));
