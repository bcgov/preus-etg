...
namespace CJG.Application.Services
{
	...
	public static class IPrincipalExtensions
	{
		...
		public static bool CanPerformAction(this IPrincipal user, GrantApplication grantApplication, ApplicationWorkflowTrigger trigger)
		{
			...
			var current_claim = grantApplication?.GetCurrentClaim();
			var has_submitted_a_claim = grantApplication?.HasSubmittedAClaim() ?? false;
			var has_approved_claim = (current_claim?.HasPriorApprovedClaim() ?? false) || (current_claim?.IsApproved() ?? false);
				...
				switch (trigger)
				{
					...
					case (ApplicationWorkflowTrigger.SubmitClaim):
						return isApplicationAdministrator
							...
							&& current_claim != null
							&& current_claim.ClaimState == ClaimState.Complete;
					case (ApplicationWorkflowTrigger.CancelAgreementHolder):
						return isApplicationAdministrator && !has_submitted_a_claim && !has_approved_claim;
					case (ApplicationWorkflowTrigger.EditParticipants):
						if (grantApplication.CanReportParticipants) return true;
							...
							return isApplicationAdministrator && !(current_claim?.ClaimState.In(ClaimState.Unassessed) ?? false);
						else
							return isApplicationAdministrator && (current_claim?.ClaimState.In(ClaimState.Incomplete, ClaimState.Complete) ?? true);
			...
			switch (trigger)
			{
				...
				case (ApplicationWorkflowTrigger.EditApplication): // TODO: This needs to be removed and replaced by the specific workflow triggers.
					switch (grantApplication.ApplicationStateInternal)
					{
						...
						case (ApplicationStateInternal.ChangeRequest):
							return !has_submitted_a_claim && isApplicationAdministrator || user.HasPrivilege(Privilege.AM2);
						...
						case (ApplicationStateInternal.ClaimDenied):
							return (isApplicationAdministrator && !has_approved_claim) || user.HasPrivilege(Privilege.AM4);
						default:
							return user.HasPrivilege(Privilege.AM4) || (user.HasPrivilege(Privilege.AM2) && isAssessor);
					}
				...
				case (ApplicationWorkflowTrigger.AddRemoveTrainingProvider):
					switch (grantApplication.ApplicationStateInternal)
					{
						...
						case (ApplicationStateInternal.AgreementAccepted):
						case (ApplicationStateInternal.ChangeRequestDenied):
							return (isApplicationAdministrator || user.HasPrivilege(Privilege.AM4)) && (programType == ProgramTypes.WDAService || !has_submitted_a_claim);
						case (ApplicationStateInternal.New):
						case (ApplicationStateInternal.Closed):
						case (ApplicationStateInternal.AgreementRejected):
						case (ApplicationStateInternal.CancelledByAgreementHolder):
				...
				case (ApplicationWorkflowTrigger.EditTrainingCosts):
					switch (grantApplication.ApplicationStateInternal)
					{
						...
						case (ApplicationStateInternal.ReturnedToAssessment):
							return user.HasPrivilege(Privilege.AM3) || (user.HasPrivilege(Privilege.AM2) && isAssessor);
						default:
							return user.HasPrivilege(Privilege.AM4) && !has_submitted_a_claim;
					}
					...
					switch (grantApplication.ApplicationStateInternal)
					{
						case (ApplicationStateInternal.AgreementAccepted):
							return (isApplicationAdministrator || user.HasPrivilege(Privilege.AM3))
								&& ((current_claim == null && (grantApplication.ParticipantForms.Any() && claimType == Core.Entities.ClaimTypes.SingleAmendableClaim || claimType == Core.Entities.ClaimTypes.MultipleClaimsWithoutAmendments))
									|| (claimType == Core.Entities.ClaimTypes.MultipleClaimsWithoutAmendments && (current_claim?.ClaimState.In(ClaimState.ClaimApproved, ClaimState.ClaimDenied, ClaimState.AmountOwing, ClaimState.AmountReceived, ClaimState.PaymentRequested, ClaimState.ClaimPaid) ?? true)));
						case (ApplicationStateInternal.ClaimDenied):
						case (ApplicationStateInternal.ClaimApproved):
							return isApplicationAdministrator
								&& claimType == Core.Entities.ClaimTypes.MultipleClaimsWithoutAmendments && (current_claim?.ClaimState.In(ClaimState.ClaimDenied, ClaimState.ClaimPaid, ClaimState.ClaimApproved, ClaimState.AmountOwing, ClaimState.AmountReceived, ClaimState.PaymentRequested) ?? true);
						default:
							return false;
					}
					...
					switch (grantApplication.ApplicationStateInternal)
					{
						...
						case (ApplicationStateInternal.ClaimReturnedToApplicant):
							return isApplicationAdministrator
								...
								&& (current_claim?.ClaimState.In(ClaimState.Incomplete, ClaimState.Complete) ?? true);
						case (ApplicationStateInternal.ClaimAssessEligibility):
							return user.HasPrivilege(Privilege.AM4) || (user.HasPrivilege(Privilege.AM2) && isAssessor);
						case (ApplicationStateInternal.ClaimAssessReimbursement):
				...
				case (ApplicationWorkflowTrigger.ReturnToAssessment):
					return !grantApplication.ApplicationStateInternal.In(ApplicationStateInternal.ApplicationDenied)
						? user.HasPrivilege(Privilege.AM4) || user.HasPrivilege(Privilege.AM3) && isAssessor && !has_approved_claim
				...
				case (ApplicationWorkflowTrigger.CancelAgreementMinistry):
					return user.HasPrivilege(Privilege.AM4) || ((user.HasPrivilege(Privilege.AM3) && isAssessor) && !has_approved_claim);
				...
				case (ApplicationWorkflowTrigger.AmendClaim):
					return claimType == Core.Entities.ClaimTypes.SingleAmendableClaim && ((isApplicationAdministrator && (current_claim?.ClaimState.In(ClaimState.ClaimDenied) ?? false))
						|| (user.HasPrivilege(Privilege.AM2, Privilege.AM5) && isAssessor && !(current_claim?.ClaimState.In(ClaimState.Incomplete, ClaimState.Complete, ClaimState.Unassessed) ?? true))
						|| (user.HasPrivilege(Privilege.AM3) && !(current_claim?.ClaimState.In(ClaimState.Incomplete, ClaimState.Complete, ClaimState.Unassessed) ?? true)));
				...
				case (ApplicationWorkflowTrigger.CloseClaimReporting):
					return (user.HasPrivilege(Privilege.AM3) || (user.HasPrivilege(Privilege.AM2, Privilege.AM5) && isAssessor)) && has_submitted_a_claim;
