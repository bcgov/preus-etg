...
namespace CJG.Application.Services
{
	...
	public class NotificationService : Service, INotificationService
	{
		...
		public IEnumerable<NotificationQueue> SendNotifications(GrantApplication grantApplication, ApplicationStateInternal originalState, DateTime appDate = default)
		{
			...
			var notificationQueue = grantApplication.NotificationQueue
				.Where(q => q.State != NotificationState.Sent && (q.SendDate == null || q.SendDate <= appDate))
				.ToArray();

			foreach (var notification in notificationQueue)
			{
				var recipient = notification.GrantApplication.BusinessContactRoles.FirstOrDefault();
				if (recipient == null)
					continue;

				var recipientsEmail = recipient.User.EmailAddress;
				if (recipientsEmail != notification.EmailRecipients)
					recipient.User.EmailAddress = notification.EmailRecipients;
