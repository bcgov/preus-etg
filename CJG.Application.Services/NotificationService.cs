...
namespace CJG.Application.Services
{
	...
	public class NotificationService : Service, INotificationService
	{
		...
		public PageList<NotificationQueue> GetNotifications(int page, int quantity, NotificationFilter filter)
		{
			...
			// Filter
			if (!String.IsNullOrWhiteSpace(filter.NotificationType)) query = query.Where(n => n.NotificationType.Caption == filter.NotificationType);
			if (!String.IsNullOrWhiteSpace(filter.Organization)) query = query.Where(n => n.Organization.LegalName == filter.Organization);
			...
			if (!String.IsNullOrWhiteSpace(filter.Search)) query = query.Where(n => (n.EmailSubject.Contains(filter.Search) || n.NotificationType.Caption.Contains(filter.Search) || n.Organization.LegalName.Contains(filter.Search)));
		...
		{
			var query = _dbContext.NotificationQueue.Where(nt => nt.GrantApplication.GrantOpening.GrantStream.GrantProgramId == grantProgramId);
			return query.Select(o => o.Organization).Distinct().ToArray();
		}
		...
		{
			var query = _dbContext.NotificationQueue.Where(nt => nt.GrantApplication.GrantOpening.GrantStream.GrantProgramId == grantProgramId);
			return query.Select(o => o.NotificationType).Distinct().ToArray();
		}
		...
		public IEnumerable<NotificationQueue> SendNotifications(GrantApplication grantApplication, ApplicationStateInternal originalState, DateTime appDate = default)
		{
			...
			if (appDate == default) appDate = DateTime.UtcNow;
			...
			foreach (var notification in grantApplication.NotificationQueue.Where(q => q.State != NotificationState.Sent && (q.SendDate == null || q.SendDate <= appDate)).ToArray())
			{
				...
				if (recipientsEmail != notification.EmailRecipients) notification.GrantApplication.BusinessContactRoles.FirstOrDefault().User.EmailAddress = notification.EmailRecipients;
		...
		public int QueueScheduledNotifications(DateTime appDate = default)
		{
			...
			var implementedGrantPrograms = _dbContext.GrantPrograms.Where(g => g.DateImplemented != null).ToArray();
			...
			if (failed > 0) throw new NotificationException($"Notifications failed to be added to the queue ({failed} failures), review the log for additional information.");
			return count;
		}
		...
		public NotificationQueue GenerateNotificationMessage(GrantApplication grantApplication, User user, GrantProgramNotificationType grantProgramNotificationType)
		{
			...
			var model = new Notifications.NotificationViewModel(grantApplication, user, _httpContext);
		...
		private PageList<NotificationQueue> GetNotifications(int page, int quantity, NotificationFilter filter, IQueryable<NotificationQueue> query)
		{
			...
			if (!String.IsNullOrWhiteSpace(filter.NotificationType)) query = query.Where(n => n.NotificationType.Caption == filter.NotificationType);
			if (!String.IsNullOrWhiteSpace(filter.Organization)) query = query.Where(n => n.Organization.LegalName == filter.Organization);
			if (!String.IsNullOrWhiteSpace(filter.Status)) query = query.Where(n => n.State.ToString() == filter.Status);
