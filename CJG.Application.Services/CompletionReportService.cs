...
namespace CJG.Application.Services
{
	public class CompletionReportService : Service, ICompletionReportService
	{
		...
		#region Methods
		/// <summary>
		/// Get the most current active and effective completion report
		/// </summary>
		/// <returns></returns>
		public CompletionReport GetCurrentCompletionReport()
		{
			return _dbContext.CompletionReports.AsNoTracking().Where(cr => cr.IsActive && cr.EffectiveDate < DateTime.UtcNow).OrderByDescending(cr => cr.EffectiveDate).First();
		}
		...
		public bool AllParticipantsHaveCompletedReport(int[] participantsOnClaim, int completionReportId)
		{
			// Need to get the id of the first question for the completion report
			var questionId = GetCompletionReportQuestionsForStep(completionReportId, 1).First().Id;
		...
		public IEnumerable<T> GetCompletionReportGroups<T>(int grantApplicationId, Func<CompletionReportGroup, T> select)
		{
			...
			return _dbContext.CompletionReportGroups.AsNoTracking().Where(o => o.ProgramTypeId == null || o.ProgramTypeId == programTypeId).OrderBy(o => o.RowSequence).Select(select).ToArray();
		}
		...
		public CompletionReportGroup GetCompletionReportGroup(int completionReportGroupId)
		{
			return _dbContext.CompletionReportGroups.FirstOrDefault(o => o.Id == completionReportGroupId);
		}
		#endregion
	}
}
