...
namespace CJG.Application.Services
{
	...
	public class GrantApplicationService : Service, IGrantApplicationService
	{
		...
		/// <summary>
		/// Get the deafult application type.
		/// </summary>
		/// <returns></returns>
		public ApplicationType GetDefaultApplicationType()
		{
			return _dbContext.ApplicationTypes.FirstOrDefault(at => at.IsActive == true);
		}
		...
		public GrantApplication Update(GrantApplication grantApplication, ApplicationWorkflowTrigger trigger = ApplicationWorkflowTrigger.EditApplication, Func<ApplicationWorkflowTrigger, bool> canPerformAction = null)
		{
			...
			{
				grantApplication.ApplicationStateExternal = grantApplication.IsSubmitable() ? ApplicationStateExternal.Complete : ApplicationStateExternal.Incomplete;
		...
		public void UpdateDeliveryDates(GrantApplication grantApplication)
		{
			...
			{
				_noteService.AddDateChangedNote(grantApplication, $"Delivery Start Date", originalStartDate, grantApplication.StartDate);
			}
			if (grantApplication.EndDate != originalEndDate)
			{
				_noteService.AddDateChangedNote(grantApplication, $"Delivery End Date", originalEndDate, grantApplication.EndDate);
			}
		...
		public void ChangeApplicationAdministrator(GrantApplication grantApplication, int? applicantId)
		{
			...
			var newName = string.Format("{0} {1}", applicant.FirstName, applicant.LastName);
			var oldName = string.Format("{0} {1}", grantApplication.ApplicantFirstName, grantApplication.ApplicantLastName);

			// We should change this in the future to support switching a specific AA, as we can support multiple AAs.
			// I think if we are only changeing the AA, this for loop should be used, foreach (var b in grantApplication.BusinessContactRoles.Where(bcr => bcr.EmployerEnrollmentId == null).ToList())
		...
		{
			var grantApplicationWithTrainingProviders =
			...
			var grantApplicationWithTrainingProgramProviders =
				 _dbContext.GrantApplications
		...
		/// <summary>
		/// Get the number of GrantApplication objects in the datasource for the specified <typeparamref name="User"/>.
		/// </summary>
		/// <param name="user"></param>
		/// <returns></returns>
		public int GetGrantApplicationsCount(User user)
		{
			if (user == null)
				throw new ArgumentNullException(nameof(user));

			return (from bcr in _dbContext.BusinessContactRoles
					where bcr.UserId == user.Id
					select bcr.GrantApplicationId).Distinct().Count();
		}
		...
		public int GetApplicationsCountByFiscal(GrantApplication grantApplication)
		{
			...
			return _dbContext.GrantApplications.Where(x => x.GrantOpening.GrantStream.GrantProgramId == grantApplication.GrantOpening.GrantStream.GrantProgramId &&
															...
															states.Contains(x.ApplicationStateInternal)).Count();
		}
		...
		public decimal GetApplicationsCostByFiscal(GrantApplication grantApplication)
		{
			...
			return _dbContext.GrantApplications.Where(x => x.GrantOpening.GrantStream.GrantProgramId == grantApplication.GrantOpening.GrantStream.GrantProgramId &&
															x.OrganizationId == grantApplication.OrganizationId &&
															x.GrantOpening.TrainingPeriod.FiscalYearId == grantApplication.GrantOpening.TrainingPeriod.FiscalYearId &&
															states.Contains(x.ApplicationStateInternal)).ToList().Sum(y => y.TrainingCost.TotalAgreedMaxCost);
		}
		...
		/// <summary>
		/// Enables or disables the specified grant applications scheduled notfications.
		/// </summary>
		/// <param name="grantApplication"></param>
		...
		public void EnableScheduledNotifications(GrantApplication grantApplication, bool enable = true)
		{
			...
			_dbContext.GrantApplications.FirstOrDefault(ga => ga.Id == grantApplication.Id).ScheduledNotificationsEnabled = enable;
		}
		...
		public PageList<GrantApplication> GetGrantApplications(int page, int quantity, ApplicationFilter filter)
		{
			...
			{
				var include = filter.States.Where(s => s.Include).Select(s => s.State).ToArray();
				if (include.Length > 0) query = query.Where(ga => include.Contains(ga.ApplicationStateInternal));

				var exclude = filter.States.Where(s => !s.Include).Select(s => s.State).ToArray();
				if (exclude.Length > 0) query = query.Where(ga => !exclude.Contains(ga.ApplicationStateInternal));
			}

			if (!String.IsNullOrWhiteSpace(filter.FileNumber)) query = query.Where(ga => ga.FileNumber.Contains(filter.FileNumber));
			if (filter.AssessorId.HasValue) query = query.Where(ga => ga.AssessorId == filter.AssessorId);
			if (filter.TrainingPeriodId.HasValue) query = query.Where(ga => ga.GrantOpening.TrainingPeriodId == filter.TrainingPeriodId);
			else if (filter.FiscalYearId.HasValue) query = query.Where(ga => ga.GrantOpening.TrainingPeriod.FiscalYearId == filter.FiscalYearId);
			if (filter.GrantStreamId.HasValue) query = query.Where(ga => ga.GrantOpening.GrantStreamId == filter.GrantStreamId);
			else if (filter.GrantProgramId.HasValue) query = query.Where(ga => ga.GrantOpening.GrantStream.GrantProgramId == filter.GrantProgramId);
			if (!String.IsNullOrWhiteSpace(filter.Applicant)) query = query.Where(ga => ga.OrganizationLegalName.Contains(filter.Applicant));
			if (filter.IsAssigned.HasValue && filter.IsAssigned.Value) query = query.Where(ga => ga.AssessorId != null);
			else if (filter.IsAssigned.HasValue && !filter.IsAssigned.Value) query = query.Where(ga => ga.AssessorId == null);
			...
			if (filter.OrderBy?.Any(x => x.Contains(nameof(GrantApplication.StateChanges))) ?? false)
			{
				...
				{
					query = query.Distinct().OrderByDescending(g => g.StateChanges.OrderByDescending(sc => sc.ChangedDate).FirstOrDefault().ChangedDate).Skip((page - 1) * quantity).Take(quantity);
				}
				else
				{
					query = query.Distinct().OrderBy(g => g.StateChanges.OrderByDescending(sc => sc.ChangedDate).FirstOrDefault().ChangedDate).Skip((page - 1) * quantity).Take(quantity);
				}
			}
			else
			{
