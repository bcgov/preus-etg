	...
	public class GrantApplicationService : Service, IGrantApplicationService
	{
		...
		public void UpdateDeliveryDates(GrantApplication grantApplication)
		{
			...
			if (grantApplication.StartDate != originalStartDate)
				_noteService.AddDateChangedNote(grantApplication, "Training Start Date", originalStartDate, grantApplication.StartDate);

			if (grantApplication.EndDate != originalEndDate)
				_noteService.AddDateChangedNote(grantApplication, "Training End Date", originalEndDate, grantApplication.EndDate);
		...
		public IOrderedQueryable<GrantApplication> GetGrantApplications(int trainingProviderInventoryId, string search)
		{
			...
			var primaryProvidersWithChanges = trainingProviders
				.Where(t => t.TrainingProviderInventoryId == trainingProviderInventoryId)
				.Where(t => t.TrainingProviderInventoryId != null)
				.Where(t => t.OriginalTrainingProviderId != null)
			...
			var providerGrantApplicationList = new Dictionary<TrainingProvider, GrantApplication>();
			...
			foreach (var p in primaryProvidersWithNoChanges)
			{
				...
				if (application != null)
					providerGrantApplicationList.Add(provider, application);
			}

			foreach (var p in primaryProvidersWithChanges)
			{
				var newProvider = _dbContext.TrainingProviders.FirstOrDefault(t => t.Id == p.Id);
				var oldProvider = _dbContext.TrainingProviders.FirstOrDefault(t => t.Id == p.Value);

				var application = oldProvider?.TrainingProgram?.GrantApplication;
		...
		public void ReturnUnderAssessmentToNew(GrantApplication grantApplication)
		{
			if (grantApplication == null)
				throw new ArgumentNullException(nameof(grantApplication));

			CreateWorkflowStateMachine(grantApplication).ReturnUnderAssessmentToNew();
		}
		...
		{
			return _dbContext.GrantApplications
				       .Where(x => x.Id != grantApplicationId
				                   && (x.ApplicantMailingAddressId == addressId
