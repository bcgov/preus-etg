...
namespace CJG.Application.Services
{
	public class ParticipantService : Service, IParticipantService
	{
		...
		public void ApproveDenyParticipants(int grantApplicationId, Dictionary<int?, bool?> participantsApproved)
		{
			...
			var reconstituteAgreeCost = false;
			var agreedParticipants = grantApplication.TrainingCost.AgreedParticipants;

			if (agreedParticipants == 0 && totalApproved > 0)
				reconstituteAgreeCost = true;

			//if the total approved does not match the agreed participant count
			if (totalApproved != agreedParticipants)
			{
				...
				var eligibleCosts = grantApplication.TrainingCost
					.EligibleCosts
					.ToList();

				var agreedParticipantsDoNotMatch = eligibleCosts
					.Count(w => w.AgreedMaxParticipants != agreedParticipants);

				if (agreedParticipantsDoNotMatch == 0 || totalApproved == 0)
				{
					...
					{
						ec.AgreedMaxParticipants = Math.Min(ec.EstimatedParticipants, totalApproved);
						ec.AgreedMaxParticipantCost = ec.AgreedMaxParticipantCost;
						if (reconstituteAgreeCost)
		...
		public void ReportAttendance(GrantApplication grantApplication, Dictionary<int, bool?> participantAttended, INoteService noteService)
		{
			...
			if (noteService != null && isExternalUser && grantApplication.ApplicationStateInternal == ApplicationStateInternal.ClaimReturnedToApplicant)
				noteService.GenerateUpdateNote(grantApplication, true);
