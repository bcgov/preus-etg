	...
	public class ApplicationWorkflowStateMachine : Service
	{
		...
		private void ConfigureStateTransitions()
		{
			...
			_stateMachine.Configure(ApplicationStateInternal.New)
				...
				.OnEntryFrom(ApplicationWorkflowTrigger.ReturnUnderAssessmentToNew, t => OnUnderAssessmentReturnedToNew())
				.Permits(_grantApplication.ApplicationStateInternal);
		...
		private void OnUnderAssessmentReturnedToNew()
		{
			_grantApplication.RemoveAssignedAssessor();
			_grantApplication.ResetCipsCode();
			...
			_logger.Info($"Grant application {_grantApplication.Id} has been returned to New.");
		}
		...
		{
			// Please note, this method is duplicated in ApplicationJobWorkflowStateMachine. Keep in sync.
		...
		private void OnSubmitClaim(Claim claim)
		{
			try
			{
				...
				if (claim.ApplicantUpdatedNotes)
				{
					var noteMessage = $"Applicant Notes: changed to '{claim.ApplicantNotes}'";
					_noteService.AddNote(claim.GrantApplication, NoteTypes.ED, noteMessage);
				}
		...
		private void OnReturnClaimToApplicant(Claim claim, string reason, IClaimService service)
		{
			...
			claim.ApplicantUpdatedNotes = false;
		...
		public void ReturnUnderAssessmentToNew()
		{
			CanPerformTrigger(ApplicationWorkflowTrigger.ReturnUnderAssessmentToNew);
			_stateMachine.Fire(ApplicationWorkflowTrigger.ReturnUnderAssessmentToNew);
		}
		...
		private void LogStateChanges(string noteReason = "", string stateChangeReason = "", string suffix = null, bool addReason = true)
		{
			...
			switch (_grantApplication.ApplicationStateInternal)
			{
				case ApplicationStateInternal.New:
					...
					if (_originalState == ApplicationStateInternal.UnderAssessment)
						break;
