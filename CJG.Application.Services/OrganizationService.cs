	...
	public class OrganizationService : Service, IOrganizationService
	{
		...
		public int NotSubmittedGrantApplicationsForUser(int orgId, Guid userBCeID)
		{
			...
			var applicationCount = (from p in _dbContext.ProgramDescriptions
					join g in _dbContext.GrantApplications on p.GrantApplicationId equals g.Id
					join n in _dbContext.NaIndustryClassificationSystems on p.TargetNAICSId equals n.Id
					where n.NAICSVersion == 2012
					      && g.ApplicantBCeID == userBCeID && g.OrganizationId == orgId
		...
		public dynamic GetOrganizationYTD(int organizationId)
		{
			...
			var query = (from tc in _dbContext.TrainingCosts
					join ga in _dbContext.GrantApplications on tc.GrantApplicationId equals ga.Id
			...
			var queryClaims = (from ga1 in _dbContext.GrantApplications
					join cl in _dbContext.Claims on ga1.Id equals cl.GrantApplicationId
					where ga1.OrganizationId == organizationId
			...
			dynObj.TotalRequested = !query.Any() ? 0 : query.Sum(q => q.TotalEstimatedReimbursement);
			dynObj.TotalApproved = !query.Any() ? 0 : query.Sum(q => q.CalculateApprovedAmount());
