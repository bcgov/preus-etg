...
namespace CJG.Application.Services
{
	...
	public class StaticDataService : Service, IStaticDataService
	{
		...
		/// <summary>
		/// Get all the martial status that are active.
		/// </summary>
		/// <returns></returns>
		public IEnumerable<MartialStatus> GetMartialStatuses()
		{
			return Get<MartialStatus, int>().Where(e => e.IsActive);
		}

		/// <summary>
		/// Get the specified martial status.
		/// </summary>
		/// <param name="id"></param>
		/// <returns></returns>
		public MartialStatus GetMartialStatus(int id)
		{
			return _dbContext.MartialStatuses.Find(id);
		}
		...
		{
			if (id == 0)
				return _dbContext.FiscalYears.AsNoTracking().Where(x => x.StartDate <= AppDateTime.UtcNow && x.EndDate >= AppDateTime.UtcNow).FirstOrDefault();
			return _dbContext.FiscalYears.Find(id);
		}


		/// <summary>
		/// Get all the training periods.
		/// This method needs to be tightened up!
		/// </summary>
		/// <returns></returns>
		public IEnumerable<TrainingPeriod> GetTrainingPeriods()
		{
			return Get<TrainingPeriod>().OrderBy(fy => fy.StartDate).ThenBy(fy => fy.Caption);
		}
		...
		{
			return Get<TrainingPeriod>().Where(tp => tp.FiscalYearId == fiscalYearId).OrderBy(fy => fy.StartDate).ThenBy(fy => fy.Caption);
		}
		...
		{
			return GetRateFormats().FirstOrDefault(r => Math.Abs(r.Rate - rate) < TypeExtensions.FloatTollerance);
		}
